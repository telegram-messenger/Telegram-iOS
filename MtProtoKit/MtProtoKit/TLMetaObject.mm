...
int32_t TLMetaObject::getInt32(int32_t name)
{
    ...
    switch (values->at(it->second).type)
    {
        case TLConstructedValueTypePrimitiveInt32:
            return values->at(it->second).primitive.int32Value;
        case TLConstructedValueTypePrimitiveInt64:
            return (int32_t)(values->at(it->second).primitive.int64Value);
        case TLConstructedValueTypePrimitiveDouble:
            return (int32_t)values->at(it->second).primitive.doubleValue;
        case TLConstructedValueTypePrimitiveBool:
            return values->at(it->second).primitive.boolValue ? 1 : 0;
        default:
            break;
    }
...
int64_t TLMetaObject::getInt64(int32_t name)
{
    ...
    {
        case TLConstructedValueTypePrimitiveInt32:
            return (int64_t)values->at(it->second).primitive.int32Value;
        case TLConstructedValueTypePrimitiveInt64:
            return (values->at(it->second).primitive.int64Value);
        case TLConstructedValueTypePrimitiveDouble:
            return (int64_t)values->at(it->second).primitive.doubleValue;
        case TLConstructedValueTypePrimitiveBool:
            return values->at(it->second).primitive.boolValue ? 1 : 0;
        default:
            break;
...
double TLMetaObject::getDouble(int32_t name)
{
    ...
    {
        case TLConstructedValueTypePrimitiveInt32:
            return (double)values->at(it->second).primitive.int32Value;
        case TLConstructedValueTypePrimitiveInt64:
            return (double)(values->at(it->second).primitive.int64Value);
        case TLConstructedValueTypePrimitiveDouble:
            return values->at(it->second).primitive.doubleValue;
        case TLConstructedValueTypePrimitiveBool:
            return values->at(it->second).primitive.boolValue ? 1 : 0;
        default:
            break;
...
bool TLMetaObject::getBool(int32_t name)
{
    ...
    {
        case TLConstructedValueTypePrimitiveInt32:
            return values->at(it->second).primitive.int32Value != 0;
        case TLConstructedValueTypePrimitiveInt64:
            return values->at(it->second).primitive.int64Value != 0;
        case TLConstructedValueTypePrimitiveDouble:
            return values->at(it->second).primitive.doubleValue != 0.0;
        case TLConstructedValueTypePrimitiveBool:
            return values->at(it->second).primitive.boolValue;
        default:
            break;
    }
...
NSData *TLMetaObject::getBytes(int32_t name)
{
    ...
    switch (values->at(it->second).type)
    {
        ...
        {
            id object = values->at(it->second).nativeObject;
...
id TLMetaObject::getObject(int32_t name)
{
    ...
    switch (values->at(it->second).type)
    {
        ...
        case TLConstructedValueTypePrimitiveInt32:
        {
            return [NSNumber numberWithInt:values->at(it->second).primitive.int32Value];
        }
        case TLConstructedValueTypePrimitiveInt64:
        {
            return [NSNumber numberWithLongLong:values->at(it->second).primitive.int64Value];
        }
        case TLConstructedValueTypePrimitiveDouble:
        {
            return [NSNumber numberWithDouble:values->at(it->second).primitive.doubleValue];
        }
        case TLConstructedValueTypePrimitiveBool:
        {
            return [NSNumber numberWithBool:values->at(it->second).primitive.boolValue];
        }
        default:
            break;
    }
